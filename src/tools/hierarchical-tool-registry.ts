    ...
    public async registerDiscoveryTools(): Promise<void> {
        ...
        this.mcpServer.registerTool(
            "execute-sap-operation",
            {
                ...
                description: "[LEVEL 3 - EXECUTION] AUTHENTICATION REQUIRED: Perform CRUD operations on SAP entities using authenticated user context. Requires valid JWT token for authorization. Use get-entity-metadata (Level 2) first to understand entity schema, then call this to execute operations. Operations execute under user's SAP identity with full audit trail. IMPORTANT FOR CREATE: Before creating a header-level entity (e.g. purchase requisition, purchase order), call get-service-metadata with format='navigation-properties-only' to discover child entities that can be deep-inserted in a single POST. Use the navigation property names (e.g. 'to_PurchaseReqnItem') as keys in the parameters payload with value { \"results\": [...] } to create header + items + sub-items in one call.",
                inputSchema: {
                    ...
                    parameters: z.record(z.any()).optional().describe("Operation parameters such as keys, filters, and data. For read-single/update/delete operations, include the entity key properties. For create/update operations, include the entity data fields. For deep-insert creates, include child entities using navigation property names as keys (discoverable via get-service-metadata with format='navigation-properties-only'), e.g. { \"PurReqnDescription\": \"My PR\", \"to_PurchaseReqnItem\": { \"results\": [{ \"Material\": \"123\", \"RequestedQuantity\": \"1\", \"to_PurchaseReqnItemText\": { \"results\": [] } }] } }."),
        ...
        this.mcpServer.registerTool(
            "get-service-metadata",
            {
                ...
                description: `Retrieve the OData $metadata for a SAP service, exposing information not available through the standard entity tools. Returns: all Function Imports and their parameters (operations not available via CRUD); navigation properties (relationships between entities); entity key properties; draft-enabled indicators. Use 'function-imports-only' format to discover what actions exist before calling execute-function-import. Use 'navigation-properties-only' format before any create operation to discover which child entities can be deep-inserted alongside the parent (e.g. items, account assignments, texts) in a single POST call. Uses technical user (no auth needed).`,
                inputSchema: {
                    serviceId: z.string().describe("SAP service ID from discover-sap-data"),
